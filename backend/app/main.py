...
class GenerateBody(BaseModel):only_missing:bool=False
...
def generate_answers(cid:int,qid:int,body:GenerateBody|None=None,db:Session=Depends(get_db)):
    ...
    progress=start_generation(item.id,cid,only_missing=bool(body and body.only_missing))
...
def export(cid:int,qid:int,db:Session=Depends(get_db)):
    item=db.scalar(select(Questionnaire).where(Questionnaire.id==qid,Questionnaire.customer_id==cid).options(selectinload(Questionnaire.questions).selectinload(Question.answer)))
    if not item:raise HTTPException(404,"Not found")
    return StreamingResponse(export_xlsx(item,False),media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",headers={"Content-Disposition":f'attachment; filename="questionnaire-{qid}.xlsx"'})
...
def export_internal(cid:int,qid:int,db:Session=Depends(get_db)):
    item=db.scalar(select(Questionnaire).where(Questionnaire.id==qid,Questionnaire.customer_id==cid).options(selectinload(Questionnaire.questions).selectinload(Question.answer)))
    if not item:raise HTTPException(404,"Not found")
    return StreamingResponse(export_xlsx(item,True),media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",headers={"Content-Disposition":f'attachment; filename="questionnaire-{qid}-internal-review.xlsx"'})

PROVIDERS=["mock","openai","azure_openai","anthropic","gemini","openrouter","bedrock","ollama","lm_studio","openai_compatible","enterprise"]
EMBEDDINGS=["mock","openai","azure_openai","openrouter","ollama","sentence_transformers","bge","e5","custom"]
